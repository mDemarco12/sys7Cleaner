...
| `css/system7.css` | All chrome: 1-bit dither pattern (shared `--dither-pattern` custom property, used by `.dither-bg`/`body`/`.splash-overlay` — the overlay needs its **own opaque fill**, not a transparent view onto body's, since it sits in front of `.mac-window`'s opaque white), title bar (`.title-bar.active` shows stripes+boxes), `.boot-section` (staged reveal), `.icon-grid`/`.icon-item` (folder/file browser), `.modal-*`, `.alert-*` (default-ring is a **separate outline outside** the button, not a thicker border — and goes on Cancel for destructive alerts, never the affirmative button). Carries the file's one documented colour exception — see "De-branding & 1-bit" below. |
| `js/icons.js` | All inline-SVG glyphs: `FOLDER_ICON_SVG`, `DOCUMENT_ICON_SVG`, `CAUTION_ICON_SVG`, `SYS7_EMBLEM_SVG` (original splash mark — **do not replace with a real OS-vendor or fictional-brand logo**; see README Branding section for why), plus five per-file-type glyphs (`fileIconSvg()`/`iconForPath()`, driven by `file-types.js`'s extension lookup). |
| `js/file-types.js` | `fileTypeForPath(path)` — the single extension→type lookup (`doc`/`photo`/`script`/`app`/`xml`) `icons.js` renders from. Unmatched or dot-less names fall back to `doc`. |
| `js/splash.js` | One-time CRT-power-on boot sequence: gray screen → box unfolds with a static flash → chromatic-aberration glitch on the emblem/text → text types itself out with a blinking cursor → one more glitch pulse + scanline sweep → fade out → `.boot-section` elements reveal staggered → `.booted` lands on `<body>` and all boot-only classes are stripped. Total time is pinned by `TOTAL_MS`, not by stacking phase durations — repack the beats, don't extend the budget. |
...
| `js/window-drag.js` | `makeWindowDraggable(windowEl, handleEls, boundsEl)` — makes an internal floating panel (a modal) draggable within `boundsEl`'s bounds, independent of the outer OS window (which moves separately via `data-tauri-drag-region` on the main title bar; the two mechanisms are deliberately never on the same element). Used by `targets.js` and `about.js`. |
...
| `js/targets.js` | `TargetPicker` — owns the catalog checkbox state (`Set` of selected ids), custom-folder add/remove via the native picker, "Deselect All". Exposes `getSelectedIds()`/`ensureLoaded()`/`getSelectedTargets()`/`onChange()`/`onOpen()` for `app.js`. Its modal is draggable via `window-drag.js`. |
| `js/sprite.js` | **Generated** by `sprite/build_sprite.py` — do not hand-edit. One `SYS7_MASCOT_SVG` constant (the About-box mascot); eye cells carry `class="sprite-eye"` so `system7.css` can drive the blink independently of the idle-bob on the rest of the sprite. |
| `js/about.js` | Open/close, renders `SYS7_MASCOT_SVG` into `#about-sprite` once at load, external-link interception (must route through `window.__TAURI__.opener.openUrl`, or a plain `<a target="_blank">` navigates the whole app away instead of opening the system browser). Its modal is draggable via `window-drag.js`. |
...
- **De-branding & 1-bit**: no Apple assets/copy remain (see README's Branding section) — don't reintroduce a real OS vendor's or another franchise's logo/copy when touching the splash or chrome. `system7.css` is otherwise strictly 1-bit (pure `#000` on `#d8d8d8`/`#fff`, no gray fills, no shadows, no gradients other than the hard-edged pinstripe) with exactly **two** bounded exceptions, both confined to modals that get torn down or stay closed by default rather than the window chrome the user actually works in: (1) the boot splash's CRT-glitch intro (`--ghost-red`/`--ghost-cyan`, `.splash-logo-ghost`, the `.splash-text` aberration keyframes), confined to `#splash`, which `splash.js` removes from the DOM entirely once boot finishes; (2) the About box's animated mascot (`SYS7_MASCOT_SVG`, `.about-sprite`), confined to `#about-modal`. Both are original art/audio — see `js/icons.js`'s and `sprite/build_sprite.py`'s own comments for the "inspired by a genre, not a copy of any specific work" reasoning. Don't extend colour usage past those two surfaces, and don't add a third exception without a similarly hard boundary.
- **Boot audio**: `src/audio/boot-neonrain.wav` (an original synthesized composition, not a sample — the generating script isn't part of this repo) plays once per launch via `splash.js`, wrapped in a `.catch(() => {})` since WebKit's autoplay policy can in principle block it even for local bundled content. This is the app's only sound; nothing else should start playing audio without a similarly deliberate, scoped decision.
