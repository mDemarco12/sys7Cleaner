...
| `js/about.js` | Trivial open/close + external-link interception (must route through `window.__TAURI__.opener.openUrl`, or a plain `<a target="_blank">` navigates the whole app away instead of opening the system browser). Its modal is draggable via `window-drag.js`. |
...
- **De-branding & 1-bit**: no Apple assets/copy remain (see README's Branding section) — don't reintroduce a real OS vendor's or another franchise's logo/copy when touching the splash or chrome. `system7.css` is otherwise strictly 1-bit (pure `#000` on `#d8d8d8`/`#fff`, no gray fills, no shadows, no gradients other than the hard-edged pinstripe) with exactly **one** bounded exception — the boot splash's CRT-glitch intro (`--ghost-red`/`--ghost-cyan`, `.splash-logo-ghost`, the `.splash-text` aberration keyframes). It's confined to `#splash`, which `splash.js` removes from the DOM entirely once boot finishes, so nothing color/gradient-based reaches the window chrome the user actually works in. Don't extend colour usage past that overlay, and don't add a second exception without a similarly hard boundary.
